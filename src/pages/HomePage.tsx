...
const HomePage = (props: Props) => {
  ...
  const toggleActiveSlide = (e: React.MouseEvent<HTMLLIElement, MouseEvent>) =>
    setActiveSlide(Number(e.currentTarget.id));

  return (
    <>
      <section className=''>
        <div className='py-14 lg:h-[100vh] lg:max-h-[600px] sm:py-20 px-[10vw] sm:px-36 bg-gray-900 text-white bg-cover flex items-center  after overflow-hidden relative'>
          <img
          ...
          <article className='relative z-10 max-w-xl flex flex-col justify-start gap-2 sm:gap-4'>
            ...
            <Link
              to={sliderContent.at(activeSlide)?.link || '/our-work'}
              className='mt-6 py-3 px-4 bg-slate-900 hover:bg-slate-600 flex self-start items-center gap-3 text-lg text-slate-200 font-light sm:px-6'
            >
              <span>
                {sliderContent.at(activeSlide)?.linkText || 'See our portfolio'}
              </span>
              <IoArrowForward />
            </Link>
          </article>
        ...
        <ul className='z-[3] absolute flex gap-0 translate-y-[-30%]  lg:translate-y-[-90%] xl:translate-x-[-80px]'>
          {sliderContent.map((_, i) => (
            <li
              ...
              className={`h-20 w-20 py-8 px-8 bg-slate-200 text-xl transition-colors duration-300 font-light tracking-widest ${
                i === activeSlide && 'bg-slate-800 text-white'
              }`}
            >
              {`0${i + 1}`}
            </li>
      ...
      <section className='flex relative lg:grid justify-center lg:grid-cols-5 gap-6 lg:gap-14 lg:grid-rows-1'>
        <article className='mt-10 xl:mt-0 flex flex-col gap-4 col-start-2 col-span-3 row-span-1 max-w-[350px] sm:max-w-none sm:w-1/3 md:w-3/6 lg:w-full'>
          <h3
            className=' hidden lg:block absolute z-[-1] leading-[0] text-[14rem] top-[15%]  left-0 text-slate-100 xl:translate-x-[-80px]'
            role='presentation'
          >
            Welcome
          </h3>
      ...
      <section className='flex gap-6 flex-col'>
        <div className='flex justify-between gap-3 items-center flex-wrap'>
          ...
          <Link
            to='/our-work'
            className='py-3 px-4 bg-slate-900 hover:bg-slate-600 flex self-center items-center gap-3 text-lg text-slate-200 font-light sm:px-6 my-auto'
          >
            <span>See all</span>
            <IoArrowForward />
          </Link>
